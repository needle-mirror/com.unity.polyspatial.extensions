    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 Frag(Varyings input) : SV_Target
            {
                ...
                // Sample each neighboring vertex depth and if there is depth average it with the other samples.
                ...
                // If this sample is centered on an object, we only average neigboring samples once.
                // This small amount of averaging between disparate depths helps reduce visible
                // distortion in reprojeciton when moving your head quickly
                if (centerIsObject)
                {
                    const float averagedDepth = cumulativeSampleCount > 0 ? cumulativeDepth / (float)cumulativeSampleCount : centerDepth;
                    return float4(averagedDepth, 0, 0, 1);
                }

                // If this sample is in empty space, we expand the neighbor samples one more vertex to reach farther out and
                // blur even more to avoid any artifacts at the edges of objects
                UNITY_UNROLL
                for (int i = 0; i < sampleOffsetCount; ++i)
                {
